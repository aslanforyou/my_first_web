...
function getHomeNames() {
    ...
    for (let i = 0; i < home.length; i++) {
        n1.push(home[i].homeName);
    }
    return n1;
}


function searchHome(home, value) {
...
//console.log('AAAAA', document)
document.addEventListener("DOMContentLoaded", function () {
    //console.log('WWindow was loaded');
    for (var i = 0; i < home.length; i++) {
        var el = home[i].homeName;
        VT.addEl('#home2', `<option value="${el}"> ${el} </option>`, false);
        VT.addEl('#DropDown', `<li><a id="${home[i].id}" href="#">${el}</a></li>`, false);

    }

    DropDown.addEventListener('click', setText);

    text.addEventListener('keydown', function () {
        VT.getEl("#btn").className = "btn btn-default";
        document.querySelector("div.input-group").className = "input-group";
    });

    VT.getEl('#btn').addEventListener('click', () => {
        var newName = document.getElementById("text").value;
        if (!newName.trim()) {
            VT.getEl("#btn").className = "btn btn-dangerr";
            document.querySelector("#form>div").className = "input-group has-error";
            return;
        }

        for (var i = 0; i < home.length; i++) {
            if (home[i].id === select) {
                home[i].homeName = text.value;
                setNewText();
            }
        }
...
function setText(e) {
    ...
    select = e.target.id;
    VT.setParam('#DropDown', 'was_selected', e.target.id);
    e = e.target.innerText;

    menu = VT.getEl("#dropdownMenuButton");
    menu.firstElementChild.value = e;
    menu.firstElementChild.innerText = e;

    VT.getEl("#text").value = e;
